...
const StrokeOrderTeacher = function (descriptorList, strokeWidth) {
...
async function requestXml(word, penWidth, onStrokeDefinationComplete) {
  if (word !== null || word !== "") {
    let code = word.split("")[0].charCodeAt(0).toString(16).toUpperCase();
    let urls = [
      "https://stroke-order.learningweb.moe.edu.tw/provideStrokeInfo.do?ucs=" +
        code +
        "&useAlt=0",
        ...
        await new Promise((resolve, reject) => {
          $.ajax({
            ...
            success: function (xml) {
              ...
              let strokeNodeList = $(xml).find("Stroke");
              let strokeDescriptorList = new Array();
              strokeNodeList.each(function () {
                ...
                outlineChilds.each(function () {
                  ...
                  if (builderFunc != null) {
                    ...
                    console.log(
                      "Command [ " + $(this).prop("tagName") + " ] not found !"
                    );
                  }
                });
...
export function initStroke($, url, doSuccess) {
  ...
  function onStrokeDefinationComplete(strokeDescriptorList, penWidth) {
    ...
    $("#svg1").svg(function (svgRoot) {
      ...
      if (correctImg == null) {
        correctImg = svgRoot.image(
          ...
          width,
          width,
          str_Url + "stroke_exercise_resources/images/effect_success.png"
        );
        $(correctImg).css("pointer-events", "none");
        $(correctImg).attr("opacity", 0);
      }
      if (errorImg == null) {
        errorImg = svgRoot.image(
          0,
          0,
          width,
          width,
          str_Url + "stroke_exercise_resources/images/effect_err.png"
        );
    ...
    pane2.addEventListener("CORRECT", function (src) {
      clearInterval(demo.drawStrokeInterval);
      doSuccess && doSuccess();
      $(correctImg).attr("opacity", 1);
      step = -0.01;
      chTime = 0;
      if (correctInterval != null) clearInterval(correctInterval);
